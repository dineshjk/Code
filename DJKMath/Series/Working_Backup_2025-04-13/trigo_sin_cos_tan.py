...
# from math_utils import operate_menu, get_single_key, get_integer, get_float, wait_for_key

from Utils import operate_menu, get_single_key, get_integer, get_float, wait_for_key
#from Utils import keyboard_utils, inout,menu_utils   # Everything from __all__ in __init__.py is imported
...
def tangent_power_series(x, precision):
    ...
    if k % 2 != 0:  # odd multiple
        expected = (k * mp.pi) / 2
        if abs(x - expected) < mpf(10) ** (-precision + 5):  # within precision threshold
...
def flprint(value, precision, message = ""):
    ...
    print(f"{message} {mp.nstr(value, n=precision, strip_zeros=False, min_fixed=-inf, max_fixed=inf)}")
...
def main():
    ...
    while True:
        ...
        if selected_func == "sine_power_series":
            ...
            #print(f"       Difference: {mp.nstr(diff, n=precision, strip_zeros=False, min_fixed=-inf, max_fixed=inf)}")
